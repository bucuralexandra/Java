...
public class Account {
    ...
    public Account(String owner, long account, float initial) {
        ...
        acctNumber = account;
        if (initial < 0)
            throw new NumberFormatException("Invalid balance input");
        else
            balance = initial;
    }
    ...
    public boolean withdraw(float amount, float fee) {
        // validate parameters
        boolean isValidW = isValidWithdrawal(amount, fee);
        if (isValidW) {
            amount = amount + fee;
            balance = balance - amount; // this can cause the balance to becoome negative
        }
        return isValidW;
        //return isValidWithdrawal(amount, fee); // we should not perfom again this method because amount variable has changed
    }

    /* Determine if withdrawal parameters are valid */
    private boolean isValidWithdrawal(float amount, float fee) //changed here to catch account+fee >= balance
    {
        return amount >= 0 && fee >= 0 && amount <= balance && fee <= balance && amount + fee <= balance;
    }
    ...
    public float getBalance() {
        return (float) Double.parseDouble(new DecimalFormat("##.####").format(balance));
    }
